...
import { buildInitialData, defaultTitleForRecordType, getFieldsForRecordType } from "@/features/records/forms/formDefs";
...
export default function AddPetRecordScreen() {
  ...
  const fields = getFieldsForRecordType(rtype);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        if (!petEntityId || !rtype) return;

        // Guard: SINGLE types should edit if already exists
        if (isSingletonType(rtype)) {
          const existing = (await listRecordsForEntity(petEntityId)).find((r) => r.recordType === rtype);
          if (existing) {
            router.replace({
              pathname: "/(vault)/pets/[petId]/records/[recordId]/edit",
              params: { petId: petEntityId, recordId: existing.id },
            } as any);
            return;
          }
        }
  ...
  }, [petEntityId, rtype, meta?.label, router]);
  ...
  const handleSave = async () => {
    ...
    await upsertRecordForEntity(petEntityId, record);
                    ...
                    {f.type === "document" ? (
                      <View>
                        <TouchableOpacity
                          ...
                          activeOpacity={0.85}
                        >
                          <Text className="text-foreground font-medium">Upload Document</Text>
