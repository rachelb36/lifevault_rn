...
export default function DependentDetailScreen() {
  ...
  }, [displayName, person?.relationship, person?.dob, ageLabel]);
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 128 }} keyboardShouldPersistTaps="handled">
          ...
          <View className="p-6">
            <View className="bg-card rounded-2xl p-6 items-center shadow-sm">
              ...
              <Text className="text-muted-foreground mb-1">{person?.relationship || "Relationship not set"}</Text>
              <Text className="text-sm text-muted-foreground mb-4">{person?.dob ? `Born: ${person.dob}` : "DOB not set"}</Text>
          ...
          <View className="px-6 mb-4">
            ...
            <Text className="text-sm text-muted-foreground mb-3">Choose how this person is related to you.</Text>
            ...
            {isEditingRelationship && relationshipDirty && (
          ...
          <View className="px-6">
            ...
            {visibleCategoryOrder.map((category) => (
              <RecordSection
                ...
                onOpen={(record) => {
                  router.push({
                    pathname: "/people/[personId]/records/[recordId]",
                    params: { personId: String(depId), recordId: record.id },
                  } as any);
                }}
              />
