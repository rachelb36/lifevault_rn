...
function buildRecordSummary(
  ...
  if (
    recordType === RECORD_TYPES.PASSPORT ||
    recordType === RECORD_TYPES.PASSPORT_CARD
  ) {
  ...
  if (recordType === RECORD_TYPES.BIRTH_CERTIFICATE) {
    ...
    return (
      [name, dob ? `DOB ${formatDateLabel(dob, dob)}` : ""]
        .filter(Boolean)
        .join(" • ") || "Added"
    );
  }
  ...
  if (recordType === RECORD_TYPES.MEDICAL_PROFILE) {
    ...
    const allergies = Array.isArray((data as any).allergies)
      ? (data as any).allergies.length
      : 0;
    const parts: string[] = [];
  ...
  if (recordType === RECORD_TYPES.PRESCRIPTIONS) {
    const list = Array.isArray((data as any).prescriptions)
      ? (data as any).prescriptions
      : [];
    if (list.length === 0) return "Added";
    const names = list
      .map((x: any) => String(x.medicationName || "").trim())
      .filter(Boolean)
      .slice(0, 3);
  ...
  if (recordType === RECORD_TYPES.VACCINATIONS) {
    const list = Array.isArray((data as any).vaccinations)
      ? (data as any).vaccinations
      : [];
  ...
  if (recordType === RECORD_TYPES.LOYALTY_ACCOUNTS) {
    ...
    if (names.length === 0)
      return accounts.length ? `${accounts.length} accounts` : "Added";
    return extra > 0 ? `${names.join(" • ")} +${extra}` : names.join(" • ");
  }

  if (recordType === RECORD_TYPES.TRAVEL_IDS) {
    const ids = Array.isArray((data as any).travelIds)
      ? (data as any).travelIds
      : [];
    const labels = ids
  ...
  if (recordType === RECORD_TYPES.PET_VACCINATIONS) {
    ...
    return (
      [name, date ? formatDateLabel(date, date) : ""]
        .filter(Boolean)
        .join(" • ") || "Added"
    );
  }
  ...
  if (recordType === RECORD_TYPES.PET_INSURANCE) {
    ...
    return (
      [provider, policy ? `#${policy}` : ""].filter(Boolean).join(" • ") ||
      "Added"
    );
  }
  ...
  if (record.updatedAt)
    return `Updated ${formatDateLabel(record.updatedAt, "—")}`;
  return "Added";
}
...
export default function SectionRecordRows({
  ...
  const filledCount = useMemo(() => {
    return recordTypes.filter((rt) =>
      categoryRecords.some((r) => r.recordType === rt),
    ).length;
